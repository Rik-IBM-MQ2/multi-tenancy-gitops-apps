...
if [ -z ${GIT_OLD_ORG} ]; then echo "Please set GIT_OLD_ORG when running script, optional GIT_BASEURL and GIT_REPO to formed the git url GIT_BASEURL/GIT_ORG/*"; exit 1; fi
...
GIT_GITOPS_MQ_INFRA=${GIT_GITOPS_MQ_INFRA:-mq-infra.git}
GIT_GITOPS_SPRING_APP=${GIT_GITOPS_SPRING_APP:-mq-spring-app.git}
...
while IFS= read -r -d '' File; do
  if grep -q "kind: Application" "$File"; then
    echo "$File"
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS}#" $File
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS_INFRA}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_INFRA}#" $File
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS_SERVICES}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_SERVICES}#" $File
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS_APPLICATIONS}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_APPLICATIONS}#" $File
    rm "${File}.bak"
  fi
done
...
find ${ROOTDIR}/ -name '*.yaml' -print0 |
while IFS= read -r -d '' File; do
  if grep -q "kind: TriggerTemplate" "$File"; then
    echo "$File"
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS_MQ_INFRA}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_MQ_INFRA}#" $File
    sed -i'.bak' -e "s#${GIT_BASEURL}/${GIT_OLD_ORG}/${GIT_GITOPS_SPRING_APP}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_SPRING_APP}#" $File
    rm "${File}.bak"
  fi
done
